...
func TestLookup_UnknownModelFallsBackToBillablePrice(t *testing.T) {
	...
	if spec != Lookup("gpt-5.6-sol") {
		t.Fatalf("unknown model pricing = %+v, want GPT-5.6-Sol pricing", spec)
	}
}
...
func TestLookup_ByKeyword(t *testing.T) {
	cases := []struct {
		...
		{"codex 系列已退役 → gpt-5.6-sol", "gpt-5.9-codex-preview", "gpt-5.6-sol"},
		{"移除的 gpt-5.3-codex-spark → gpt-5.6-sol", "gpt-5.3-codex-spark", "gpt-5.6-sol"},
		...
		{"未知 gpt-5 系列 → gpt-5.6-sol", "gpt-5.9", "gpt-5.6-sol"},
		{"移除的 gpt-5.4 → gpt-5.6-sol", "gpt-5.4", "gpt-5.6-sol"},
		{"o1 推理模型 → gpt-5.6-sol", "o1-preview", "gpt-5.6-sol"},
		{"o3 mini 推理模型 → gpt-5.6-luna", "o3-mini", "gpt-5.6-luna"}, // "mini" 优先
		{"gpt-4 系列 → gpt-5.6-sol", "gpt-4o", "gpt-5.6-sol"},
	}
...
func TestLookup_KnownModelUnchanged(t *testing.T) {
	t.Run("gpt-6", func(t *testing.T) {
		cases := []struct {
			...
			{model: "gpt-6-luna", input: 0.1, cached: 0.01, cacheCreation: 0.125, output: 0.5, contextWindow: 1050000},
		}
		for _, tc := range cases {
