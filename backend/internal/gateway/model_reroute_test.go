...
func TestForwardOAuthUnknownModelRequestsReroute(t *testing.T) {
	...
	if target, ok := outcome.ModelRerouteClientTarget(); !ok || target != "gpt-5.6-sol" {
		t.Fatalf("Forward() outcome = %+v, want gpt-5.6-sol model reroute", outcome)
	}
...
func TestOAuthModelRerouteOutcome(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "removed GPT-5.4 reroutes to GPT-5.6-Sol",
		...
		},
		{
			...
			req:  oauthModelRerouteTestRequest("codex-auto-review", "gpt-5.6-sol", "gpt-5.6-sol"),
			path: "/v1/responses",
		},
		{
			name: "wire model controls resolution independently of scheduling pool",
			req:  oauthModelRerouteTestRequest("codex-auto-review", "review-pool", "gpt-5.6-sol"),
			path: "/v1/responses",
		},
		{
